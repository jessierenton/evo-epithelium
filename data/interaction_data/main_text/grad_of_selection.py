...
def gos_weak_selection(b,Lambda_CC):
    n=np.arange(1,100,dtype=float)
    return np.hstack([0,(n/N**2)*DELTA*(-c*(N-n)+b*(Lambda_CC*N-n)),0])
...
if __name__ == '__main__':    
    ...
    for b in np.arange(1.5,4.5,0.5):
        plt.scatter(np.hstack([0,x,1]),gos_weak_selection(b,Lambda_CC)*10**(3),label=r'$b=%.1f$'%b,marker='o',s=8)
